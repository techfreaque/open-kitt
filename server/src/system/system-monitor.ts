...
import { SystemInfo, NetworkStatus } from './system-types.js';
...
async function getNetworkStatus(): Promise<NetworkStatus> {
  try {
    ...
    const activeInterface = networkInterfaces.find(iface => 
      iface.operstate === 'up' && (iface.type === 'wireless' || iface.type === 'wired')
    );
    ...
    if (activeInterface.type === 'wireless') {
      try {
        ...
        logger.error(`Failed to get SSID: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
    ...
    logger.error(`Failed to get network status: ${error instanceof Error ? error.message : String(error)}`);
    return { connected: false, type: 'unknown', ip: '', ssid: '' };
  }
}
...
async function collectSystemInfo(): Promise<SystemInfo> {
  try {
    ...
    logger.error(`Failed to collect system info: ${error instanceof Error ? error.message : String(error)}`);
    return {
      ...systemInfo,
      time: new Date().toISOString()
    };
 ...
 */
export async function executeSystemCommand(command: string): Promise<{ success: boolean; output: string }> {
  try {
