...
import type { CanMessage, CanStatus } from "./can-types.js";
 ...
 */
async function checkCanInterface(
  interfaceName: string = "can0",
): Promise<CanStatus> {
  try {
    // Check if interface exists
    const { stdout: ipLinkOutput } = await execAsync(
      `ip -d link show ${interfaceName}`,
    );
    ...
    logger.error(
      `Failed to check CAN interface: ${error instanceof Error ? error.message : String(error)}`,
    );
    return {
      connected: false,
      interface: interfaceName,
 ...
 */
async function bringUpCanInterface(
  interfaceName: string = "can0",
  ...
  try {
    ...
    logger.info(
      `CAN interface ${interfaceName} brought up with bitrate ${bitrate}`,
    );
    return true;
  } catch (error) {
    logger.error(
      `Failed to bring up CAN interface: ${error instanceof Error ? error.message : String(error)}`,
    );
    return false;
  }
}
...
export async function setupCanBus(io: SocketIOServer): Promise<void> {
  ...
  if (!canStatus.connected) {
    logger.warn(
      `CAN interface ${canStatus.interface} is not connected. Attempting to bring it up...`,
    );
...
function initializeCanChannel(io: SocketIOServer): void {
  try {
    ...
    logger.error(
      `Failed to initialize CAN channel: ${error instanceof Error ? error.message : String(error)}`,
    );
    canStatus.connected = false;
  ...
  try {
    ...
    const hexData =
      data
        ...
        ?.join(" ") || "";
    await execAsync(
      `cansend ${canStatus.interface} ${id.toString(16)}#${hexData}`,
    );
    return true;
  } catch (error) {
    logger.error(
      `Failed to send CAN message: ${error instanceof Error ? error.message : String(error)}`,
    );
    return false;
  }
}
