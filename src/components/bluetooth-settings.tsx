...
export function BluetoothSettings({ className }: BluetoothSettingsProps) {
  ...
  const disconnectFromDevice = async (device: BluetoothDevice) => {
    ...
    try {
      // Call the backend API to disconnect from the device
      ...
      if (data.success) {
        ...
        throw new Error(data.error || "Failed to disconnect from device");
      }
    } catch (error) {
  ...
  const pairDevice = async (device: BluetoothDevice) => {
    ...
    try {
      // Call the backend API to pair with the device
      ...
      if (data.success) {
        ...
        throw new Error(data.error || "Failed to pair with device");
      }
    } catch (error) {
      console.error("Failed to pair with device:", error);
      // Show error to user
      alert(
        `Failed to pair: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
  ...
  const unpairDevice = async (device: BluetoothDevice) => {
    ...
    try {
      // Call the backend API to unpair the device
      const response = await fetch("/api/bluetooth/unpair", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!response.ok) {
        throw new Error(`Unpairing failed: ${response.statusText}`);
      }
      ...
      // Show error to user
      alert(
        `Failed to unpair: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
