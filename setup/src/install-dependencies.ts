...
async function installDependencies() {
  ...
  try {
    ...
    await execa('bash', ['-c', 'curl -fsSL https://deb.nodesource.com/setup_20.x | bash -']);
    ...
    log.error(`Failed to install dependencies: ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  }
}
