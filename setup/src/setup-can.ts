...
async function setupCAN() {
  log.info(
    `Starting CAN bus setup for ${options.interface} with bitrate ${options.bitrate}...`,
  );
  ...
  try {
    ...
    try {
      ...
      modulesSpinner.succeed(
        "CAN kernel modules loaded including MCP2515 support",
      );
    } catch (error) {
      modulesSpinner.warn(
        "CAN kernel modules loaded, but MCP2515 module not available",
      );
    }

    // Set up CAN interface
    const interfaceSpinner = ora(
      `Setting up ${options.interface} with bitrate ${options.bitrate}...`,
    ).start();
    try {
      await execa("ip", [
        "link",
      ...
      interfaceSpinner.succeed(
        `${options.interface} configured and brought up`,
      );
    } catch (error) {
      ...
      log.warn(
        "This could be because the interface does not exist or is already configured.",
      );
    ...
    const serviceSpinner = ora(
      "Creating systemd service for CAN setup...",
    ).start();
...
`;
    ...
    try {
      ...
      testSpinner.warn(
        "No CAN messages received in 5 seconds. This is normal if no device is sending messages.",
      );
    }
