...
program
  .option(
    ...
    process.env.SUDO_USER || process.env.USER || "root",
  )
...
`;
      ...
      await fs.writeFile(
        "/etc/systemd/system/open-car.service",
        serviceContent,
      );
...
`;
    ...
    await fs.writeFile(
      "/etc/NetworkManager/conf.d/99-opencar.conf",
      networkConfig,
    );
    ...
    if (options.autostart) {
      log.info(
        "The system will automatically start the OpenCar dashboard on next boot.",
      );
      log.info(
        `You can manually start it with: systemctl start open-car.service`,
      );
    } else {
      log.info(
        `You can start the application manually with: cd ${projectRoot} && pm2 start ecosystem.config.json`,
      );
    }
  } catch (error) {
    log.error(
      `Failed to set up system: ${error instanceof Error ? error.message : String(error)}`,
    );
    process.exit(1);
  }
}
